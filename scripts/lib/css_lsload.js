...
function cssHelper() {
  ...
  for (var i = 0, len = arguments.length; i < len; i++) {
    ...
    if (Array.isArray(path)) {
      ...
      if (
        path.indexOf("?") < 0 &&
        path.substring(path.length - 4, path.length) !== ".css"
      )
        path += ".css";
      ...
      result +=
        '<style id="' +
        key +
        '"></style><script>if(typeof window.lsLoadCSSMaxNums === "undefined")window.lsLoadCSSMaxNums = 0;' +
        "window.lsLoadCSSMaxNums++;" +
        'lsloader.load("' +
        key +
        '","' +
        require("../../../../node_modules/hexo/dist/plugins/helper/url_for").call(
          this,
          path,
        ) +
        (fs.existsSync(localpath) ? "?" + get_file_hex(localpath) : "") +
        '",function(){if(typeof window.lsLoadCSSNums === "undefined")window.lsLoadCSSNums = 0;' +
